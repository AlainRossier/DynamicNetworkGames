...
vector<double> initproba(int n) {
  if(n<2) {return vector<double>{1} ;}
  else {
    vector<double> res(n) ; 
    random_device rd;  //Will be used to obtain a seed for the random number engine
    mt19937 gen(rd()); //Standard mersenne_twister_engine seeded with rd()
    uniform_real_distribution<> dis(0.0, 1.0);
    vector<double> cuts(0) ; //stores the values outputed by the generator
    for (int m = 0; m < n-1; ++m) {
      //Use dis to transform the random unsigned int generated by gen into a double in [0, 1)
      cuts.push_back(dis(gen)); //Each call to dis(gen) generates a new random double
    }
    sort(cuts.begin(), cuts.end()) ;
    res[0] = cuts[0] ;
      for (int m = 1; m < n-1; m++) {
	res[m] = cuts[m] - cuts[m-1] ;
      }
    res[n-1] = 1 - cuts[n-2] ;
    return res ;
  }
...
void initprobaindiv(vector<vector<vector<double> > > &probaindiv) {
  for(size_t i = 0; i<probaindiv.size(); i++) {
    for(size_t g = 0; g<probaindiv[0].size(); g++) {
      probaindiv[i][g] = initproba(probaindiv[0][0].size()) ; 
    }
  }
}
